...
// Send the weak topics to our Python backend to build a dynamic 7-day plan
export const generateStudyPlan = async (weakTopics: string[]): Promise<StudyDay[]> => {
  try {
    // If they got a perfect score (no weak topics), give them an advanced plan
    const topicsToSend = weakTopics.length > 0 ? weakTopics : ["Advanced Problem Solving", "Time Management Integration"];
    ...
    const response = await fetch("http://localhost:8000/generate-plan", {
      ...
      // Fast API expects exactly this JSON structure based on our BaseModel
      body: JSON.stringify({ weak_topics: topicsToSend }), 
    });
    ...
    // Emergency Hackathon Fallback (just in case the Python server drops)
    return [
      { 
        day: 1, 
        topic: weakTopics[0] || "Core Concepts", 
        subtopics: [
          { name: "Review Fundamentals", youtubeQuery: "JEE Fundamentals", wikiTopic: "Physics" },
          { name: "Basic Problem Solving", youtubeQuery: "JEE Basic Problems", wikiTopic: "Mathematics" }
        ] 
      },
      { 
        day: 2, 
        topic: weakTopics[1] || "Intermediate Applications", 
        subtopics: [
          { name: "Formula Derivations", youtubeQuery: "JEE Formula Derivations", wikiTopic: "Chemistry" }
        ] 
      },
      { 
        day: 3, 
        ...
        subtopics: [
          { name: "Cross-topic Problems", youtubeQuery: "JEE Cross Topic Practice", wikiTopic: "Science" }
        ] 
      },
      { 
        day: 4, 
        topic: weakTopics[2] || "Advanced Problem Solving", 
        subtopics: [
          { name: "Previous Year Questions", youtubeQuery: "JEE Previous Year Questions Solving", wikiTopic: "Engineering" }
        ] 
      },
      { 
        day: 5, 
        topic: "Weak Area Targeting", 
        subtopics: [
          { name: "Error Analysis", youtubeQuery: "How to analyze mock test errors", wikiTopic: "Learning" }
        ] 
      },
      { 
        day: 6, 
        topic: "Final Brush Up", 
        subtopics: [
          { name: "Formula Memorization", youtubeQuery: "JEE Formula Revision", wikiTopic: "Memory" }
        ] 
      },
      { 
        day: 7, 
        topic: "Full JEE Mock Test", 
        subtopics: [
          { name: "3-Hour Timed Simulation", youtubeQuery: "How to attempt JEE Main paper", wikiTopic: "Test_(assessment)" }
        ] 
      }
    ];
  }
